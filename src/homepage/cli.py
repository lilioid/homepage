...
def send_webmentions(dry_run: bool):
    asyncio.run(homepage.webmention.send_webmentions(dry_run))
...
def main() -> Optional[int]:
    ...
    argp.add_argument("--db", default="sqlite:///./database.sqlite", help="sqlite connection string")
    ...
    match args.action:
        case "serve":
            app_config = AppConfig(bind=args.bind, db_uri=args.db, dev_mode=args.dev)
            run_webserver(app_config)
        case "send-webmentions":
            send_webmentions(args.dry_run)
